...
\implies C_1 (1,0,-i) + C_2(1+i,1-i,1) + C_3(i,i,i) = \vec{0}
\end{align}
So,
\begin{align}
...
C_2 = 0
\end{align}
As $C_1,C_2,C_3 \neq 0$, so $\vec{\alpha_1}$,$\vec{\alpha_2}$ and $\vec{\alpha_3}$ are linearly independent and as the dimension of the vector space $C^3$ is 3. So we can say that these 3 vectors span the vector space $C^3$ which implies that these 3 vectors form a basis of vector space $C^3$.

Now, consider a vector $\beta$ = $(a,b,c)$ such that
\begin{align}
\beta R = \myvec{a & b & c}\myvec{\rho_1\\\rho_2\\\rho_3}
\end{align}
Where $\rho_1$ = $\myvec{1 & 0 & i-1}$, $\rho_2$ = $\myvec{0 & 1+i & -1}$ and $\rho_3$ = $\myvec{0 & 0 & 2-i}$ are the rows of the matrix $R$.

Now, 
\begin{align}
\beta R = \myvec{a & b & c}Q A
\end{align}
